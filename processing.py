...
def build_transition_graph(question_list, seq_len_list, indices, student_num, concept_num):
    ...
    # 创建一共大小为（concept_num,concept_num）的全零矩阵，用于表示转移图，矩阵的行列分别对于知识点编号
    student_dict = dict(zip(indices, np.arange(student_num)))
    # 创建一个学生字典，将索引indices中的学生编号，从0开始重新关联编号，即将学生编号映射到矩阵的行索引中
    # 下面的循环时为了筛选当前时间步回答题目的学生，没答题的学生跳过
    for i in range(student_num):
        if i not in student_dict:
            continue
        questions = question_list[i] # 获取答题学生的答题序列和序列长度
        seq_len = seq_len_list[i] # 计算知识点被回答的次数
        for j in range(seq_len - 1):  # 遍历答题序列中的每一个位置
            pre = questions[j] # 当前位置问题编号
            next = questions[j + 1] # 下一个位置问题编号
            # 获取当前位置和下一个位置上的题目编号
            graph[pre, next] += 1 # 将 当前知识点 到下一个知识点 的转移频率加1
    np.fill_diagonal(graph, 0) # 对角线元素设为0，即知识点自身没有到自身的转移频率（去除环）
    # row normalization
    rowsum = np.array(graph.sum(1)) # 计算每行的转移频率之和，得到每个知识点的出度数组
    def inv(x): # 计算倒数
        if x == 0:
            return x
        return 1. / x
    inv_func = np.vectorize(inv)
    r_inv = inv_func(rowsum).flatten() # 对每行的转移频率之和取倒数
    r_mat_inv = np.diag(r_inv) # 创建一个对角矩阵，其对角线上元素为每个知识点出度的倒数
    graph = r_mat_inv.dot(graph) # 将出度的倒数矩阵左乘到转移图上，实现对转移图的归一化
    # 将每行的转移频率除以对应知识点的出度之和，得到转移概率矩阵。
