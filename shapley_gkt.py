...
class GKT(nn.Module):
    ...
    def cal_sha_adj(self, yt, pred, qt):
        ...
        concept_num = adj[0]
        positions_dict, key_to_idx, positions, coefficients, unique_inverse, unique_positions = self.construct_positions_dict(
            adj, max_order=3)
        rpositions_dict, rkey_to_idx, rpositions, rcoefficients, runique_inverse, runique_positions = self.construct_positions_dict(
            reverse_adj, max_order=3)
        ...
        key_to_val = {key: vals[unique_inverse[key_to_idx[key]]].clone().detach().to(torch.float64) for key in
                      key_to_idx}
        rkey_to_val = {key: vals[runique_inverse[rkey_to_idx[key]]].clone().detach().to(torch.float64) for key in
                       rkey_to_idx}
