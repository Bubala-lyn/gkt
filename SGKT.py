...
class GKT(nn.Module):
    ...
    def _adj(self, qt):
        ...
        print("masked_qt", qt_mask)
        masked_qt = qt[qt_mask]  # [mask_num, ]
        adj_tensor = self.graph[masked_qt.long(), :].unsqueeze(dim=-1)  # [mask_num, concept_num, 1]
        reverse_adj = self.graph[:, masked_qt.long()].transpose(0, 1).unsqueeze(dim=-1)  # [mask_num, concept_num, 1]
        # self.f_neighbor_list[0](neigh_ht) shape: [mask_num, concept_num, hidden_dim]
        return adj_tensor
    ...
    def cal_sha_adj(self, yt, pred, qt):
        ...
        adj_tensor = self._adj(qt)
        concept_num = adj_tensor[0]
        ...
        print("result", result)
        new_adj = torch.sum(result, dim=2)
        print("new_adj", new_adj.shape)
        print("new_adj", new_adj)
        return new_adj

    def construct_positions_dict(self, adj_tensor, max_order):
